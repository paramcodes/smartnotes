  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-700 px-6 py-4 flex items-center gap-4">
        <Link
          href="/dashboard"
          className="flex items-center gap-1.5 text-sm text-gray-600 dark:text-gray-300 hover:text-black dark:hover:text-white"
        >
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={label} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-4 space-y-3">
              <div className={`w-9 h-9 ${bg} dark:bg-opacity-20 rounded-xl flex items-center justify-center`}>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-4">Weekly Activity</h2>
            {data.weekActivity.every(d => d.count === 0) ? (
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-6">No activity this week</p>
            ) : (
              <div className="flex items-end gap-2 h-28">
                {data.weekActivity.map(({ day, count }) => (
                  <div key={day} className="flex-1 flex flex-col items-center gap-1">
                    <span className="text-xs text-gray-600 dark:text-gray-400">{count > 0 ? count : ''}</span>
                    <div
                      className="w-full rounded-t-md bg-black dark:bg-white transition-all"
          ...
          {/* Top tags */}
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
            <h2 className="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-4 flex items-center gap-1.5">
              <Tag size={14} /> Most Used Tags
            </h2>
            {data.topTags.length === 0 ? (
              <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-6">No tags yet</p>
            ) : (
                ...
                {data.topTags.map(({ tag, count }) => (
                  <div key={tag} className="space-y-1">
                    <div className="flex justify-between text-xs text-gray-700 dark:text-gray-300">
                      ...
                      <span className="text-gray-500 dark:text-gray-400">{count} {count === 1 ? 'note' : 'notes'}</span>
                    </div>
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
          ...
          {data.recentNotes.length === 0 ? (
            <p className="text-sm text-gray-500 dark:text-gray-400 text-center py-6">No notes yet</p>
          ) : (
            <div className="divide-y">
              {data.recentNotes.map(note => (
                <div key={note.id} className="py-3 flex items-center justify-between">
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate text-gray-900 dark:text-gray-100">{note.title || 'Untitled'}</p>
                  ...
                  <div className="flex items-center gap-2 ml-4 shrink-0">
                    {note.tags?.slice(0, 2).map((tag: string) => (
                      <span key={tag} className="text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 px-2 py-0.5 rounded-full">
                        {tag}
                      </span>
                    ))}
